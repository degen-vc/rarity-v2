...
contract RarityManifestedMarket is Ownable {
    ...
    function collectFees(address payable feeReceiver) external onlyOwner {
        require(s.feeBalance > 0, "No fee left");
        s.feeBalance = 0;
        Address.sendValue(payable(feeReceiver), s.feeBalance);
    }

    // change the fee
    function setFee(uint256 fee) public onlyOwner {
        require(fee <= 100, "don't be greater than 100%!");
        s.fee = fee;
        emit FeeChanged(s.fee);
    }

    function setMinPrice(uint256 minPrice) public onlyOwner {
        s.minPrice = minPrice;
