...
contract rarity_names is ERC721Enumerable {
    ...
    function tokenURI(uint name_id) public override view returns (string memory output) {
        ...
        output = string(abi.encodePacked('data:application/json;base64,', Base64.encode(bytes(string(abi.encodePacked('{"name": "', names[name_id], '", "description": "Scarcity ERC721 names for summoners.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))))));
    }
