...
contract NamesV3 is Ownable {
  ...
  function tokenURI(uint256 name_id) public view returns (string memory output) {
    ...
    output = string(abi.encodePacked('data:application/json;base64,', Base64.encode(bytes(string(abi.encodePacked('{"name": "', names[name_id], '", "description": "Rarity 2 ERC721 names for adventurers.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))))));
  }
