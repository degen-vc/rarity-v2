...
contract RarityCraftingIMarket is Initializable, ERC721Holder {
  ...
  function emitListInfo(uint _id) internal {
    (uint8 base_type, uint8 item_type, uint32 crafted, uint crafter, address minter) = ICrafting(address(nft))
      .items(_id);
    emit ListInfo(_id, base_type, item_type, crafted, crafter, minter);
  }
