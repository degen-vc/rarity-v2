...
describe('GoldV2', function() {
  ...
  const NAME_BUY_PRICE_WEI = utils.parseEther("1000");
  ...
  before('setup', async function() {
    ...
    const Payment_token = await ethers.getContractFactory('contracts/Mocks/MockERC20.sol:MockERC20');
    const payment_token = await Payment_token.deploy(owner.address);    

    const NamesV3 = await ethers.getContractFactory('contracts/core/NamesV3.sol:NamesV3');
    const namesV3 = await NamesV3.deploy(scarcity.address, payment_token.address, NAME_BUY_PRICE_WEI);

    const Gold = await ethers.getContractFactory('contracts/core/GoldV2.sol:GoldV2');
    gold = await Gold.deploy(scarcity.address, namesV3.address);
